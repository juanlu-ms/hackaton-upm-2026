...
import {
    CloudRain, AlertTriangle, Plus, History, RefreshCw,
    Thermometer, Wind, Droplets, Eye, Send, Users, Bell,
...
export default function BackofficePage() {
    ...
    async function loadHistory() {
        const [wRes, lRes, aRes] = await Promise.all([
            supabase.from('weather_logs').select('*').order('created_at', { ascending: false }).limit(50),
            supabase.from('llm_queries').select('*').order('created_at', { ascending: false }).limit(50),
        ...
        <div className="dashboard admin-dashboard">
            ...
            {showAlertForm && (
                <section className="dashboard-card alert-form-card">
                    <h2><Plus size={22} /> Nueva Alerta de Emergencia</h2>
                        ...
                        <div className="form-group">
                            ...
                            <div className="severity-selector">
                                {['info', 'warning', 'critical'].map((s) => (
                                    ...
                                    >
                                        {s === 'info' ? 'ℹ️ Info' : s === 'warning' ? '⚠️ Warning' : '🔴 Crítico'}
                                    </button>
                                ))}
                            </div>
                        </div>
                        <button type="submit" className="btn-danger btn-full" disabled={alertLoading}>
                            <Send size={18} />
                            {alertLoading ? 'Emitiendo...' : 'Emitir Alerta a Todos los Ciudadanos'}
                        </button>
                    </form>
                </section>
            )}
            ...
            <section className="dashboard-card">
                <div className="card-header">
                    <h2><AlertTriangle size={22} /> Análisis de Riesgo</h2>
                    <button
            ...
            <section className="dashboard-card">
                <div className="card-header">
                    <h2><History size={22} /> Historial Completo</h2>
                </div>
                                ...
                                alertHistory.map((a) => (
                                    <div key={a.id} className={`history-item ${severityClass(a.severity)}`}>
                                        <div className="history-item-header">
                                            ...
                                            <span className={`alert-status ${a.active ? 'active' : 'inactive'}`}>
